...
Public Class mlib
    ...
    Const _MinersFee As Long = 20000
    Const _Dust As Long = 6000
    ...
    Public Function multisigbarray(ByVal key As String)
        ...
        Return barray
    End Function
    Public Function rpccalld(ByVal method As String, ByVal param0 As String, ByVal param1 As String, ByVal param2 As String, ByVal param3 As String) As String
        Dim bitcoin_con As mlib.bitcoinrpcconnection = (New mlib.bitcoinrpcconnection)
        Return rpccall(bitcoin_con, method, param0, param1, param2, param3)
    End Function
    ...
    Public Function rpccall(ByVal bitcoin_con As bitcoinrpcconnection, ByVal method As String, ByVal param0 As String, ByVal param1 As String, ByVal param2 As String, ByVal param3 As String) As String
        ...
        Try
            ...
            process.StartInfo.FileName = Trim(My.Settings.BitcoindExe)
    ...
    Public Function rpccallnew(ByVal bitcoin_con As bitcoinrpcconnection, ByVal method As String, ByVal param0 As String, ByVal param1 As String, ByVal param2 As String, ByVal param3 As String) As String
        Dim Ans As String = ""
        Dim BitcoindExe As String = Trim(My.Settings.BitcoindExe)
        If InStr(BitcoindExe, "bitcoind.exe") > 0 Then
            Try
                ...
                Dim arg As String = "-rpcconnect=" + bitcoin_con.bitcoinrpcserver + " -rpcport=" + bitcoin_con.bitcoinrpcport.ToString + " -rpcuser=" + bitcoin_con.bitcoinrpcuser + " -rpcpassword=" + bitcoin_con.bitcoinrpcpassword + " " + method + " " + Trim(params)
                process.StartInfo.Arguments = arg
                process.StartInfo.UseShellExecute = False
                process.StartInfo.CreateNoWindow = True
                process.StartInfo.RedirectStandardOutput = True
    ...
    End Function
    Function jsonadd(ByVal Name As String, ByVal value As String) As String
        Return """" + Name + """:" + """" + value + """"
    End Function
    Public Function getrandombyte()
        ...
        Return sb.ToString
    End Function
    Function GetFromPubKey(ByVal fromadd As String) As String
        Dim frompubkey As String = ""
        Try
            Dim json As String = (New mlib).rpccalld("validateaddress", 1, fromadd, 0, 0)
            ...
            End If
        Catch e As Exception
            MsgBox("Exeption thrown encodetx  validating key: " & e.Message)
        End Try
        Return frompubkey
    End Function
    Function GetValidECSDA(ByVal encodedpubkey) As String
        'validate ECDSA points
        Dim isvalidecdsa = False
    ...
    End Function
    Function GetInputTXMultipleInputnotworking(ByVal fromaddress As String, ByVal Amount As Double, ByRef ChangeAmount As Double) As String
        Dim txhex As String = ""
        Dim jOut As JArray = JsonConvert.DeserializeObject(rpccalld("listunspent", 2, 1, 999999, 0))
        Dim TotalOutput As Double = 0
        Dim Ctr As Integer = 0
        For Each Tran In jOut
            If Tran.Item("address") = fromaddress Then
                TotalOutput += Val(Tran.Item("amount"))
                Ctr += 1
            End If
        Next

        If TotalOutput < Amount Then
            MsgBox("Not enough funds from " + fromaddress)
        Else
            'build tx hex raw
            txhex = "01000000" 'version
            TotalOutput = 0
            txhex += LCase(IIf(Ctr <= 15, "0", "") + Hex(Ctr)) ' "01" "0f"
            For Each Tran In jOut
                If Tran.Item("address") = fromaddress Then
                    If TotalOutput > Amount Then
                        Exit For
                    Else
                        TotalOutput += Val(Tran.Item("amount"))
                        Dim fromtxid As String = Trim(Tran.Item("txid"))
                        Dim fromtxvout As String = Tran.Item("vout")

                        txhex += txidtohex(fromtxid) 'input txid hex
                        txhex += i32tohex(fromtxvout) 'input vout 00000000
                        txhex += "00" 'scriptsig length
            ...
            'handle change
            ChangeAmount = (TotalOutput - Amount)
        End If
        Return txhex
    End Function
    Function GetInputTX(ByVal fromaddress As String, ByVal Amount As Double, ByRef ChangeAmount As Double) As String
        Dim txhex As String = ""
        Dim jOut As JArray = JsonConvert.DeserializeObject(rpccalld("listunspent", 2, 1, 999999, 0))
        Dim InputAmount As Double = 0
        Dim AmountandMiner As Double = Amount + SattoAmount(_MinersFee)
        Dim Found As Boolean = False
        txhex = "01000000" 'version
        txhex += "01" '1 input
        For Each Tran In jOut
            If Tran.Item("address") = fromaddress Then
                InputAmount = Val(Tran.Item("amount"))
                If InputAmount > AmountandMiner And InputAmount > 0.00056 Then
                    Dim fromtxid As String = Trim(Tran.Item("txid"))
                    Dim fromtxvout As String = Tran.Item("vout")
                    txhex += txidtohex(fromtxid) 'input txid hex
                    txhex += i32tohex(fromtxvout) 'input vout 00000000
        ...
        Next
        If Not Found Then
            MsgBox("No unspent Input > " + (AmountandMiner).ToString + " found.")
            txhex = ""
            ChangeAmount = 0
        Else
            ChangeAmount = Math.Round(InputAmount - AmountandMiner, 8)
        End If
        Return txhex
    End Function
    ...
    Public Function encodetx(ByVal fromadd As String, ByVal toadd As String, ByVal curtype As Integer, ByVal amount As Long)
        Dim txhex As String = ""
        Dim fromtxid As String = ""
        Dim fromtxvout As Integer = 0
        Dim fromtxamount As Long = 0
        Dim changeamount As Double = 0
        Dim totaltxfee As Long = (_Dust * 4)
        Dim encodedpubkey As String = ""
        Dim frompubkey As String = ""

        frompubkey = GetFromPubKey(fromadd)
        If frompubkey = "" Then
        ...
        Else
            ...
            encodedpubkey += "00000000" 'simple send
            encodedpubkey += i32tohexlittle(curtype)
            encodedpubkey += i64tohexlittle(amount)
            encodedpubkey += "0000000000000000000000000000" 'padding
            ...
            encodedpubkey = GetValidECSDA(encodedpubkey)

            txhex = GetInputTX(fromadd, SattoAmount(totaltxfee), changeamount)
            If txhex <> "" Then
                txhex += "04" 'number of vouts, future: cater for 3 outs (no change) - since we check txin for >totaltxfee there will always be change for now
                'change output
                txhex += i64tohex(AmounttoSat(changeamount)) 'changeamount value
                txhex += "19" 'length - 25 bytes
                txhex += "76a914" & addresstopubkey(fromadd) & "88ac" 'change scriptpubkey
                'exodus output
                txhex += i64tohex(_Dust)
                txhex += "19"
                txhex += "76a914946cb2e08075bcbaf157e47bcb67eb2b2339d24288ac" 'exodus scriptpubkey
                'reference/destination output
                txhex += i64tohex(_Dust)
                txhex += "19"
                txhex += "76a914" & addresstopubkey(toadd) & "88ac" 'data scriptpubkey 

                'multisig output
                txhex += i64tohex(_Dust * 2)
                txhex += "47" 'length - ??bytes?? calculate
                txhex += "51" '???
                txhex += "21" '???
                txhex += frompubkey 'first multisig address
                txhex += "21" '???
                txhex += LCase(encodedpubkey) 'second multisig address
                txhex += "52ae" '???
        ...
        Return txhex
    End Function
    Public Function encodeselltx(ByVal fromadd As String, ByVal curtype As Integer, ByVal saleamount As Long, ByVal offeramount As Long, ByVal minfee As Long, ByVal timelimit As Integer, ByVal Action As Integer)
        Dim txhex As String = ""
        Dim fromtxid As String = ""
        ...
        'first pubkey
        encodedpubkey = "01" 'compressedkey+seqnum
        encodedpubkey = encodedpubkey + "00010014" 'Version 1 Sell Offer 
        ...
        encodedpubkey2 += minfeestr.Substring(10, 6)
        encodedpubkey2 += "0" + Conversion.Hex(Action)
        encodedpubkey2 += "000000000000000000000000000000000000000000000000000000" 'padding
        ...
        encodedpubkey = GetValidECSDA(encodedpubkey)
        encodedpubkey2 = GetValidECSDA(encodedpubkey2)
        ...
        Else
            ...
            '       txhex += "ffffffff" 'sequence
            If txhex <> "" Then
                txhex += "03" 'number of vouts, future: cater for 3 outs (no change) - since we check txin for >totaltxfee there will always be change for now
                ...
                'multisig output
                txhex += i64tohex(_Dust * 3)
                txhex += "69" 'length - ??bytes?? calculate
                txhex += "51" '???
                txhex += "21" '???
                txhex += frompubkey 'first multisig address
                txhex += "21" '???
                txhex += encodedpubkey 'second multisig address
                txhex += "21" '???
                txhex += encodedpubkey2 'third multisig address
                txhex += "53ae" '???
                txhex += "00000000" 'locktime
                txhex = LCase(txhex)
            End If
        ...
        Return txhex
    End Function
    Public Function encodeaccepttx(ByVal fromadd As String, ByVal toadd As String, ByVal curtype As Integer, ByVal purchaseamount As Long)
        Dim txhex As String = ""
        Dim fromtxid As String = ""
        Dim fromtxvout As Integer = -1
        Dim fromtxamount As Double = -1
        ...
        Return txhex
    End Function
    Public Function encodepaymenttx(ByVal fromadd As String, ByVal toadd As String, ByVal amounttopay As Double, ByVal fees As Double) As String
        Dim txhex As String = ""
        Dim fromtxid As String = ""
        Dim fromtxvout As Integer = -1
        Dim fromtxamount As Double = -1
        Dim frompubkey As String = ""
        Dim ChangeAmount As Double = 0
        Dim totaltxfee As Double = amounttopay + SattoAmount(_Dust)
        ...
        If frompubkey = "" Then
            MsgBox("Error locating public key for from address.")
        Else
            txhex = GetInputTX(fromadd, totaltxfee, ChangeAmount)
            If txhex <> "" Then
                txhex += "03" 'number of vouts
                'change output
                txhex += i64tohex(AmounttoSat(ChangeAmount)) 'changeamount value
                txhex += "19" 'length - 25 bytes
                txhex += "76a914" & addresstopubkey(fromadd) & "88ac" 'change scriptpubkey
                ...
                txhex += "76a914946cb2e08075bcbaf157e47bcb67eb2b2339d24288ac" 'exodus scriptpubkey
                'reference/destination output
                txhex += i64tohex(amounttopay * 100000000)
                txhex += "19"
                txhex += "76a914" & addresstopubkey(toadd) & "88ac" 'data scriptpubkey 
                txhex += "00000000" 'locktime
            End If
    ...
    Public Function StringToByteArray(ByVal hex As [String]) As Byte()
        ...
        Return bytes
    End Function
    Public Function encodepaymenttx2old(ByVal bitcoin_con As bitcoinrpcconnection, ByVal fromadd As String, ByVal toadd As String, ByVal amounttopay As Double, ByVal fees As Double) As String
        Dim txhex As String = ""
        Dim fromtxid As String = ""
        Dim fromtxvout As Integer = -1
        Dim fromtxamount As Double = -1
        Dim SatChangeamount As Long = 0
        Dim totaltxfee As Long = 26000 'include 0.0002 miner fee
        Dim SatDust As Long = 6000
        Dim frompubkey As String = ""
        Dim SatAmounttoPay As Long = amounttopay * 100000000
        Dim SatFees As Long = fees * 100000000
        '        Dim TotalOut As Double = amounttopay + fees + (SatDust * 3 / 100000000)
        Dim TotalOut As Double = amounttopay + fees + (SatDust / 100000000)
        ...
        Else
            ...
            For Each Tran In jOut
                ...
                If Val(Tran.Item("value")) > TotalOut * 100000000 Then
            ...
            If fromtxid = "" Or fromtxvout < 0 Or fromtxamount < 0 Then
                MsgBox("Insufficient funds from " + fromadd)
            Else
                '                Dim temp As String = (New mlib).rpccall(bitcoin_con, "settxfee", 1, fees, 0, 0)
                'build tx hex raw
                txhex = "01000000" 'version
                TotalIn = fromtxamount / 100000000
                ...
                'handle change
                SatChangeamount = (TotalIn - TotalOut) * 100000000
                txhex += "03"

                'change output
                txhex += i64tohex(SatChangeamount) 'changeamount value
                txhex += "19" 'length - 25 bytes
                txhex += "76a914" & addresstopubkey(fromadd) & "88ac" 'change scriptpubkey
                'exodus output
                txhex += i64tohex(SatDust)
                txhex += "19"
                txhex += "76a914946cb2e08075bcbaf157e47bcb67eb2b2339d24288ac" 'exodus scriptpubkey
                'reference/destination output
                txhex += i64tohex(SatAmounttoPay)
                txhex += "19"
                txhex += "76a914" & addresstopubkey(toadd) & "88ac" 'data scriptpubkey 
        ...
        Return txhex
    End Function
    Function GetFromPubKey2(ByVal fromadd As String) As String
        Dim frompubkey As String = ""
        Try
            Dim json As String = (New mlib).rpccalld("validateaddress", 1, fromadd, 0, 0)
            Dim validate As JObject = JsonConvert.DeserializeObject(json)
            ...
            With validate
                '.Item("result")
                frompubkey = .Item("pubkey")
                IsCompressed = .Item("iscompressed").ToString = "True"
            End With
